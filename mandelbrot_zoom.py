...
mandelbrot = mandelbrot_set(xmin, xmax, ymin, ymax, width, height, maxiter, method=method)
im = plt.imshow(mandelbrot, interpolation='none', animated=True)
...
def animate(i):
    ...
    mandelbrot = mandelbrot_set(xmini, xmaxi, ymini, ymaxi, width, height, maxiter, method=method)
    im.set_array(getattr(cm, colormap)(mandelbrot))
    return [im]
